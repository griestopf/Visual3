...
![alt text](01_simple_bone_rig.png)

The above image shows a simple rig consisting of an armature with a hierarchy chain of four bones controlling a cucumber-shaped mesh. The mesh has an armature modifier applied that connects the mesh to the armature. The innermost bone, "Bone.004" has an Inverse Kinematic (IK) constrained applied. The IK chain has a length of 3 bending all bones but the outermost ("Bone"). The geometry mesh is currently in "Weight Paint" mode with the innermost bone selected; thus the color encodes for each vertex how much the vertex is following the bone "Bone.004" (red vertices: completely following the bone; black vertices: not affected by the bone at all).